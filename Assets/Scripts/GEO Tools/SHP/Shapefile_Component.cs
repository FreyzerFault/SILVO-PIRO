...
namespace SILVO.GEO_Tools.SHP
{
    ...
    public class Shapefile_Component: MonoBehaviour
    {
        ...
        // For Lazy Loading when Shapefile is read
        ...
        private void Update()
        {
            // Si no se ha cargado pero tiene un path, cargarlo
        ...
        public static Shapefile_Component InstantiateShapefile(string filePath, int maxSubPolygonCount = PolygonRenderer.DEFAULT_MAX_SUBPOLYGONS_COUNT,
            bool onTerrain = false, int terrainOffset = 200)
        {
            Shapefile_Component shpfileComp = UnityUtils.InstantiateObject<Shapefile_Component>(null);
            shpfileComp.maxSubPolygonCount = maxSubPolygonCount;
            shpfileComp.overTerrain = onTerrain;
            shpfileComp.terrainOffset = terrainOffset;
        ...
        public void UpdateTexture(int texRes)
            => previewTexture = GetTexture(texRes, Color.white, Color.black);

        // TODO Una imagen con todas las Shapes
        protected virtual Texture2D GetTexture(int texRes = 128, Color fillColor = default, Color backgroundColor = default)
        {
            if (_shpComponents.IsNullOrEmpty()) return null;
            return SHP_Component.GetTexture(_shpComponents.ToArray(), texRes, fillColor, backgroundColor);
        }
